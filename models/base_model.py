...
class BaseModel(ABC):
    ...
    def get_current_losses(self, is_validation=False):
        ...
        errors_ret = OrderedDict()#创建一个有序字典，用于存储损失名称和对应数值
        for name in self.loss_names:#遍历self.loss_names中的每个损失名称
            if isinstance(name, str):   #检查每个name是否为字符串类型
                if isinstance(name, str):
                    if is_validation:   #判断is_validation是否为true，如果是则在name的后面加上_validation;  但是注意此处传入通过train.py文件是个空参数，默认使用False
